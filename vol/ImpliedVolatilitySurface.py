...
1. Import the class and load your data:
   ...
   from data.import_options import import_options_data
...
class ImpliedVolatilitySurface:
    ...
    def plot(
        ...
        if save_png:
            try:
                # Create output directory (relative to script if not absolute)
...
if __name__ == "__main__":
    # Example usage with SPX options
    import sys
    import os
    
    sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    
    from data.import_options import import_sp500_options_data
    ...
    if not df.empty:
        print("\n" + "="*70)
        print("PROCESSING SPX OPTIONS")
        print("="*70)
        
        # Create IV surface calculator
        spx_iv = ImpliedVolatilitySurface(
            df=df,
            currency="SP500",
            r=0.05,
            use_mark_iv=True,
        ...
        valid_iv_count = spx_iv.df['iv_percentage'].notna().sum()
        print(f"\n✓ Valid IV data for {valid_iv_count} out of {len(spx_iv.df)} SPX options")
        print(f"✓ Spot price used: ${spx_iv.spot_price:,.2f}")
        
        # Plot continuous surface
        print("\n" + "="*70)
        print("PLOTTING SPX VOLATILITY SURFACE")
        print("="*70)
        spx_iv.plot(
            option_type="both",
            interpolate=True,
